...
code('constants','Constants', 'constants = Table([Column("key", "Symbol", math=True, align="left"), Column("value", "Value / unit"), Column("definition", "Definition / basis", align="left")], [(input_symbol(r["key"]), f"{float(r[\'value\']):g} {r[\'unit\']}", r["description"] + "; " + r["basis"]) for r in ROWS])',kind='table',meta='pagebreak=true')
...
code('fluidinputs','Water and geometry','rho_w = C["rho_w"] * kg / m**3\nmu_w = C["mu_w"] * Pa * s\nd_face = C["active_d"] * mm\nd_bore = C["stub_id"] * mm\nmdot_n = C["mdot_n"] * lb / s\nmdot_s = C["mdot_s"] * lb / s\nfluid_table = Table(["Symbol", "Value"], [(Symbol("rho_w"), rho_w), (Symbol("mu_w"), f"{C[\'mu_w\']:g} Pa s"), (Symbol("d_face"), d_face), (Symbol("d_bore"), d_bore), (Math("dot(m)_n"), mdot_n), (Math("dot(m)_s"), mdot_s)])',kind='table')
...
text('resistance_basis','Assumed resistance coefficients',body='Use the assumed values below for Dutch twill and the combined coarse pair. These are preliminary inputs, not measured supplier data. The viscous terms scale with viscosity and velocity; the inertial terms scale with density and velocity squared. Housing loss uses bore velocity: $Delta p_"body" = K_"body" rho V_"bore"^2 / 2$.')
code('hyd_inputs','Coefficient definitions / constants workbook','C_vf = C["Cv_f"] / m\nC_if = C["Ci_f"]\nC_vc = C["Cv_c"] / m\nC_ic = C["Ci_c"]\nK_body = C["K_body"]\ncoefficient_table = Table([Column("symbol", "Symbol", align="left"), Column("value", "Assumed value"), Column("definition", "Definition", align="left")], [(Symbol("C_vf"), C_vf, "Viscous resistance of the single fine Dutch-twill layer"), (Symbol("C_if"), C_if, "Dimensionless inertial resistance of the single fine layer"), (Symbol("C_vc"), C_vc, "Combined viscous resistance of both coarse supports"), (Symbol("C_ic"), C_ic, "Combined dimensionless inertial resistance of both coarse supports"), (Symbol("K_body"), K_body, "Dimensionless inlet, outlet and transition loss coefficient; excludes mesh")])',kind='table')
...
text('dirt','11 / Loaded pressure drop',True)
...
figure('detail_b','Housing and pack / section detail','detail_circle.svg',175,72,caption='Circular section from Build123d. Gold: solid frames; blue: media envelopes.')
